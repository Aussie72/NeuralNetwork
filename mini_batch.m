...
[theta_0, theta, J_history] = mini_batch_gradient_descent(x, y, theta, alpha, iterations);
...
function [theta_0, theta, J_history] = mini_batch_gradient_descent(x, y, theta, alpha, iterations)
    ...
    %% batch if random length from 1 to length(x)
    j = 2;
    ...
    for i = 1:iterations
        ...
        for j = 1:j
            ...
            t1 = theta(1) - alpha*sum(h-y);
            t2 = theta(2) - alpha*sum(h-y).*x(t(j),2);
            t3 = theta(3) - alpha*sum(h-y).*x(t(j),3);
            theta(1) = t1;
            theta(2) = t2;
            theta(3) = t3;
         end
