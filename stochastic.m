...
%% initialising number of iterations and learning_rate and later on adjusting it to get the lowest MSE%%
iterations = 2000;
alpha = 0.001;

%% calling the Batch Gradient Descent function to optimize theta %%
[theta_0, theta, J_history] = stochastic_gradient_descent(x, y, theta, alpha, iterations);
...
function [theta_0, theta, J_history] = stochastic_gradient_descent(x, y, theta, alpha, iterations)
    m = size(y);
    ...
    %% any random number from 1 to length(x)
    j = 1;
    ...
    for i = 1:iterations
        ...
        t1 = theta(1) - alpha*(h-y(t(j),1));
        t2 = theta(2) - alpha*(h-y(t(j),1)).*x(t(j),2);
        t3 = theta(3) - alpha*(h-y(t(j),1)).*x(t(j),3);
        theta(1) = t1;
        theta(2) = t2;
        theta(3) = t3;
        theta_0 = [theta_0 theta];
        J_history = [J_history, compute_cost(x,y,theta)];
