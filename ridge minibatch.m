...
%% calling the Ridge Stochastic Gradient Descent function to optimize theta %%
[theta_0, theta, J_history] = ridge_mini_batch_gradient_descent(x, y, theta, alpha, iterations, lambda);
...
%% Ridge Mini Batch Gradient Descent Function %%
function [theta_0, theta, J_history] = ridge_mini_batch_gradient_descent(x, y, theta, alpha, iterations, lambda)
    m = size(y);
    ...
    for i = 1:iterations
       ...
       for j = 1:j
           ...
           t2 = (1-alpha*lambda)*theta(2) - alpha*sum(h-y).*x(t(j),2);
           t3 = (1-alpha*lambda)*theta(3) - alpha*sum(h-y).*x(t(j),3);
           theta(1) = t1;
           theta(2) = t2;
           theta(3) = t3;
        end
