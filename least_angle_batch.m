...
%% calling the Least Angle Batch Gradient Descent function to optimize theta %%
[theta_0, theta, J_history] = least_angle_batch_gradient_descent(x, y, theta, alpha, iterations, lambda);
...
%% Least Angle Batch Gradient Descent Function %%
function [theta_0, theta, J_history] = least_angle_batch_gradient_descent(x, y, theta, alpha, iterations, lambda)
    ...
    for i = 1:iterations
        ...
        t2 = theta(2) - alpha*sum((h-y).*x(:,2)) - alpha*lambda/2*sign(theta(2));
        t3 = theta(3) - alpha*sum((h-y).*x(:,3)) - alpha*lambda/2*sign(theta(3));
        theta(1) = t1;
        theta(2) = t2;
        theta(3) = t3;
